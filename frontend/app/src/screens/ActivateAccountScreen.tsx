...
export default function ActivateAccountScreen({ navigation }: Props) {
  const { requestActivation } = useAuth();
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const { devOtp } = await requestActivation(mobile);
      navigation.navigate('Otp', { mobile, devOtp });
    } catch (err: any) {
  ...
  return (
    <KeyboardAwareScreen style={styles.screen}>
      <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        <BrandMark size={56} />
        <Text style={styles.title}>Activate{'\n'}your account.</Text>
        <Text style={styles.subtitle}>Enter the mobile number your admin registered you with — we'll text you a code to verify it</Text>
        ...
        <View style={styles.formCard}>
          ...
          <Button title="Send Verification Code" onPress={handleSubmit} loading={loading} style={styles.submitButton} />

          <View style={styles.footerLinks}>
            <TouchableOpacity onPress={() => navigation.navigate('MpinLogin')}>
              <Text style={styles.link}>Already activated? Sign in</Text>
            </TouchableOpacity>
          </View>
        </View>
      </ScrollView>
