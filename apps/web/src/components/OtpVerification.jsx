...
export default function OtpVerification() {
  ...
  // Extract phone & automatic simulation code (if passed from register/login)
  const phone = location.state?.phone || '';
  const initialAutoCode = location.state?.autoCode || '';
  
  const [code, setCode] = useState('');
  const [cooldown, setCooldown] = useState(30); // 30 seconds cooldown
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [mockMessage, setMockMessage] = useState(initialAutoCode ? `[Dev Mode] Mock SMS OTP Sent: ${initialAutoCode}` : '');
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      await mockDb.verifyOtp(phone, code);
      navigate('/profile');
    } catch (err) {
  ...
  const handleResend = async () => {
    ...
    setCooldown(30);
    try {
      const newCode = await mockDb.generateOtp(phone);
      setMockMessage(`[Dev Mode] New Mock SMS OTP: ${newCode}`);
      console.log(`[Mock SMS Backend] Resent OTP for ${phone} is: ${newCode}`);
    } catch (err) {
  ...
  return (
    <div className="form-slide">
      <h3 className="form-title">
        <ShieldCheck size={26} /> Verify Phone
      </h3>
