...
function closeAll(){document.querySelectorAll(".drawer,.scrim,.modal").forEach(e=>e.classList.remove("on"));$("#drawer").setAttribute("aria-hidden","true");}
...
$("#menuBtn").onclick=()=>{const m=$("#mobilenav");m.classList.toggle("open");$("#menuBtn").setAttribute("aria-expanded",m.classList.contains("open"));};
$("#mobilenav").innerHTML=[...document.querySelectorAll("#navlinks a")].map(a=>`<a href="${a.getAttribute("href")}">${a.textContent}</a>`).join("")+`<a href="#/wishlist">Wishlist</a><a href="#/auth">Sign in</a>`;
$("#mobilenav").onclick=e=>{if(e.target.tagName==="A")$("#mobilenav").classList.remove("open");};
